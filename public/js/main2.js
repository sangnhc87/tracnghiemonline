// public/js/main.js (Phiên bản 100% Hoàn Chỉnh - Sửa lỗi cuối cùng)

document.addEventListener('DOMContentLoaded', function() {
    // Toàn bộ code sẽ chỉ chạy sau khi DOM và các script defer đã được tải.
    
    // --- KHAI BÁO CÁC DỊCH VỤ FIREBASE ---
    // Không cần khởi tạo, chỉ cần lấy các dịch vụ từ đối tượng 'firebase' đã có sẵn
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            functions.httpsCallable("onTeacherSignIn")()
                .then(res => {
                    const data = res.data;
                    const trialDate = data.trialEndDate && data.trialEndDate.seconds ? new Date(data.trialEndDate.seconds * 1000) : new Date(data.trialEndDate);
    ...
    function gradeQuiz(isCheating = false){
        ...
        if(!isCheating){
            for(let i=0; i < examData.questionTexts.length; i++){
                ...
                else if(type === "TF"){ for(let j=0; j < examData.tfCounts[i]; j++){ if(!document.querySelector(`input[name='q${i}_sub${j}']:checked`)) unanswered.push(`Câu ${i + 1} (Ô ${j + 1})`); } }
