...
// Hãy THAY THẾ TOÀN BỘ hàm getExamStatistics cũ của bạn bằng hàm này.

exports.getExamStatistics = functions.runWith({ timeoutSeconds: 120, memory: '256MB' }).https.onCall(async (data, context) => {
    // --- BƯỚC 0: KIỂM TRA ĐẦU VÀO VÀ QUYỀN TRUY CẬP ---
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Bạn phải đăng nhập để thực hiện chức năng này.');
    }
    const teacherId = context.auth.uid;
    ...
    // --- BƯỚC 1: TRUY VẤN TẤT CẢ DỮ LIỆU THÔ TỪ FIRESTORE ---
    ...
    // Sắp xếp theo thời gian giảm dần rất quan trọng cho việc lấy bài 'mới nhất'
    const snapshot = await query.orderBy('timestamp', 'desc').get();

    if (snapshot.empty) {
        return { summary: { count: 0 }, filters: data };
    }

    const allSubmissions = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

    // --- BƯỚC 2: LỌC BÀI NỘP DỰA TRÊN TÙY CHỌN XỬ LÝ TRÙNG LẶP ---
    let filteredSubmissions = [];
    if (duplicateHandling === 'all' || !duplicateHandling) {
        // Nếu chọn 'tất cả', hoặc không có tùy chọn, lấy tất cả bài nộp
        filteredSubmissions = allSubmissions;
    } else {
        const studentSubmissionsMap = new Map();
        for (const sub of allSubmissions) {
            // Tạo một khóa duy nhất cho mỗi học sinh
            const studentKey = `${sub.studentName}||${sub.className}`;
            const existingSub = studentSubmissionsMap.get(studentKey);

            if (!existingSub) {
                // Nếu chưa có bài nộp nào của học sinh này, thêm vào map
                studentSubmissionsMap.set(studentKey, sub);
            } else {
                // Nếu đã có, kiểm tra điều kiện để ghi đè
                if (duplicateHandling === 'highest' && sub.score > existingSub.score) {
                    studentSubmissionsMap.set(studentKey, sub);
                }
                // Nếu là 'latest', vì ta đã sắp xếp theo thời gian giảm dần,
                // bài nộp đầu tiên gặp phải đã là bài mới nhất, nên không cần làm gì.
            }
        }
    ...
    // --- BƯỚC 3: LẤY ĐÁP ÁN ĐÚNG CỦA ĐỀ THI ---
    const examDoc = await db.collection('exams').where('examCode', '==', examCode).where('teacherId', '==', teacherId).limit(1).get();
    if (examDoc.empty) {
        throw new functions.https.HttpsError('not-found', `Không tìm thấy đề thi với mã ${examCode}.`);
    }
    const examData = examDoc.docs[0].data();
    const correctKeys = examData.keys; // Mảng đáp án ['A', 'B', 'TF', ...]

    // --- BƯỚC 4: KHỞI TẠO CÁC BIẾN THỐNG KÊ ---
    let totalScore = 0;
    let highestScore = -1;
    let lowestScore = 11;
    ...
    // --- BƯỚC 5: LẶP QUA DỮ LIỆU ĐÃ LỌC ĐỂ TÍNH TOÁN ---
    for (const sub of filteredSubmissions) {
        const score = sub.score;

        // a. Cập nhật thống kê tổng quan
        totalScore += score;
        ...
        // b. Cập nhật thống kê chi tiết từng câu
        // Logic này sẽ hoạt động ngay cả khi document submission không có trường `detailedResults`
        for (let i = 0; i < correctKeys.length; i++) {
            const qKey = `q${i}`;
            const analysis = questionAnalysis[qKey];
            const userAnswer = sub.answers ? sub.answers[qKey] : null;

            if (userAnswer !== null && userAnswer !== undefined) {
                analysis.totalAttempts++;

                // Ghi nhận lựa chọn của học sinh
                const choice = userAnswer === '' ? 'Bỏ trống' : userAnswer;
                analysis.choices[choice] = (analysis.choices[choice] || 0) + 1;
                
                // Kiểm tra câu trả lời đúng
                // Dùng `detailedResults` nếu có vì nó chính xác hơn (đã được chấm trên server)
                if (sub.detailedResults && sub.detailedResults[qKey] && sub.detailedResults[qKey].scoreEarned > 0) {
                    analysis.correctAttempts++;
                } 
                // Fallback: Tự so sánh đáp án nếu không có detailedResults
                else if (!sub.detailedResults && userAnswer.toString() === correctKeys[i].toString()) {
                    analysis.correctAttempts++;
                }
            }
        }
    ...
    // --- BƯỚC 6: HOÀN THIỆN VÀ TRẢ VỀ KẾT QUẢ ---
    const count = filteredSubmissions.length;
    const average = count > 0 ? totalScore / count : 0;
    
    // Sắp xếp danh sách chi tiết theo điểm giảm dần để hiển thị
