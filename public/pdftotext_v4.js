...
document.addEventListener('DOMContentLoaded', () => {
    // === NÂNG CẤP 1: BIẾN VÀ HẰNG SỐ CHO HỆ THỐNG ALIAS ===
    let teacherAlias = null; // Sẽ lưu mã của giáo viên, vd: "sangmath"
    const ALIAS_KEY = 'currentTeacherAlias';

    // Các key trong localStorage sẽ được tạo động dựa trên alias
    let DYNAMIC_DRAFTS_KEY;
    let DYNAMIC_CURRENT_FILE_ID_KEY;
    let DYNAMIC_SIDEBAR_WIDTH_KEY;
    let DYNAMIC_SIDEBAR_COLLAPSED_KEY;
    // --- DOM Elements ---
    ...
    // === NÂNG CẤP 2: HÀM KHỞI ĐỘNG CÓ KIỂM TRA ĐĂNG NHẬP ===
    function start() {
        teacherAlias = localStorage.getItem(ALIAS_KEY);
        if (teacherAlias) {
            // Đã có alias, tiến hành vào app
            loginOverlay.style.display = 'none';
            setupAppForUser();
        } else {
            // Chưa có alias, hiện form đăng nhập
            loginOverlay.style.display = 'flex';
        }
    }

    // Hàm này sẽ được gọi sau khi có alias
    async function setupAppForUser() {
        // Tạo các key động cho localStorage
        DYNAMIC_DRAFTS_KEY = `${teacherAlias}_draftsTree`;
        DYNAMIC_CURRENT_FILE_ID_KEY = `${teacherAlias}_currentFileId`;
        DYNAMIC_SIDEBAR_WIDTH_KEY = `${teacherAlias}_sidebarWidth`;
        DYNAMIC_SIDEBAR_COLLAPSED_KEY = `${teacherAlias}_sidebarCollapsed`;

        // Hiển thị nút đăng xuất
        logoutBtn.style.display = 'flex';
        logoutBtn.title = `Đăng xuất (${teacherAlias})`;

        // Gọi hàm khởi tạo chính của ứng dụng
        await initializeApp();
    }
    
    // Gọi hàm bắt đầu khi trang tải xong
    start();
    ...
    // --- NÂNG CẤP 3: SỬA CÁC HÀM DÙNG localStorage ---

// --- Data Persistence ---
function loadDraftsTree() {
    const treeData = localStorage.getItem(DYNAMIC_DRAFTS_KEY);
    draftsTree = treeData ? JSON.parse(treeData) : [];
    currentFileId = localStorage.getItem(DYNAMIC_CURRENT_FILE_ID_KEY);
}
function persistDraftsTree() { 
    localStorage.setItem(DYNAMIC_DRAFTS_KEY, JSON.stringify(draftsTree)); 
}
function persistCurrentFileId() {
    if (currentFileId) localStorage.setItem(DYNAMIC_CURRENT_FILE_ID_KEY, currentFileId);
    else localStorage.removeItem(DYNAMIC_CURRENT_FILE_ID_KEY);
}

// ... trong hàm initializeSidebarState()
function initializeSidebarState() {
    const savedWidth = localStorage.getItem(DYNAMIC_SIDEBAR_WIDTH_KEY);
    if (savedWidth) {
        sidebar.style.width = savedWidth;
    }
    if (localStorage.getItem(DYNAMIC_SIDEBAR_COLLAPSED_KEY) === 'true') {
        document.body.classList.add('sidebar-collapsed');
    }
}

// ... trong hàm stopResizing()
function stopResizing(e) {
    localStorage.setItem(DYNAMIC_SIDEBAR_WIDTH_KEY, sidebar.style.width);
    window.removeEventListener('mousemove', startResizing, false);
    window.removeEventListener('mouseup', stopResizing, false);
}

// ... trong hàm toggleSidebar()
function toggleSidebar() {
    const isCollapsed = document.body.classList.toggle('sidebar-collapsed');
    localStorage.setItem(DYNAMIC_SIDEBAR_COLLAPSED_KEY, isCollapsed);
    ...
    function initializeEventListeners() {
        // --- NÂNG CẤP 4: THÊM EVENT LISTENER CHO LOGIN/LOGOUT ---
    loginBtn.addEventListener('click', () => {
        const alias = aliasInput.value.trim().toLowerCase();
        if (alias) {
            localStorage.setItem(ALIAS_KEY, alias);
            start(); // Gọi lại hàm start để vào app
        } else {
            Swal.fire('Lỗi', 'Vui lòng nhập Mã giáo viên.', 'error');
        }
    });
    ...
    logoutBtn.addEventListener('click', () => {
        Swal.fire({
            ...
            text: `Bạn sẽ cần nhập lại mã "${teacherAlias}" để vào lại.`,
            icon: 'question',
...
/**
 * Xử lý sự kiện khi người dùng bấm nút "Xóa toàn bộ".
 * Hàm này sẽ xóa tất cả dữ liệu liên quan đến giáo viên hiện tại.
 */
function handleClearAllData() {
    Swal.fire({
        // Sử dụng teacherAlias trong câu hỏi để xác nhận đúng người dùng
        title: `Xóa toàn bộ dữ liệu của "${teacherAlias}"?`,
        text: "Toàn bộ cây thư mục và các tệp của bạn sẽ bị xóa vĩnh viễn và không thể khôi phục!",
        icon: 'warning',
        showCancelButton: true,
        ...
        // Chỉ thực hiện khi người dùng bấm nút xác nhận
        if (result.isConfirmed) {
            
            // 1. XÓA DỮ LIỆU KHỎI localStorage
            // Xóa các key động đã được tạo riêng cho giáo viên này.
            localStorage.removeItem(DYNAMIC_DRAFTS_KEY);
            localStorage.removeItem(DYNAMIC_CURRENT_FILE_ID_KEY);
            // Bạn cũng có thể xóa các cài đặt giao diện, nhưng tùy chọn
            // localStorage.removeItem(DYNAMIC_SIDEBAR_WIDTH_KEY);
            // localStorage.removeItem(DYNAMIC_SIDEBAR_COLLAPSED_KEY);

            // 2. RESET CÁC BIẾN TRẠNG THÁI TRONG BỘ NHỚ
            draftsTree = [];      // Cây thư mục giờ là mảng rỗng
            currentFileId = null; // Không có file nào đang mở
            activeNodeId = null;  // Không có mục nào đang được chọn
            
            // 3. CẬP NHẬT GIAO DIỆN
            // Đặt lại nội dung của editor về trạng thái mặc định
            if (editor) {
                editor.setValue(`\\begin{ex}\n    Dữ liệu đã được xóa. Tạo tệp mới để bắt đầu.\n\\end{ex}`);
            }

            // Vẽ lại cây thư mục (bây giờ sẽ hiển thị thông báo "Không có tài liệu")
            renderTree(draftsTree, treeContainer);

            // Cập nhật thông tin ở footer của sidebar
            updateCurrentFileInfo();

            // Hiển thị thông báo thành công
            Swal.fire(
                'Đã xóa!',
                'Toàn bộ dữ liệu của bạn đã được dọn dẹp.',
                'success'
            );
        }
    });
